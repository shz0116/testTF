...
feature_watched_values = np.random.randint(0, features, (batch * 4 * nnz, ))
row_lengths = [nnz for i in range(batch * 4)] 
# print("Feature: ", feature_watched_values)
batch_size = batch * nnz 

# feature_watched_values = [0, 3, 1, 0, 1, 1, 4, 5, 0, 2, 1, 3, 2, 2, 4, 4]
# feature_watched_row_lengths = [2, 2, 2, 2, 2, 2, 2, 2]
...
#      return dataset_ops.DatasetV2.from_tensor_slices(features).repeat().batch(batch_size)
      return dataset_ops.DatasetV2.from_tensor_slices(ragged_features).repeat().batch(batch)
    return input_fn
...
if __name__ == "__main__":
    ...
    # warmup 
    start = time.time()
    res = get_replica_numpy(test_fn(tf.constant(args.times)), strategy, 0)
    print(res[0].numpy())
    end = time.time()
    t1 = end - start
    ...
    with tf.experimental.async_scope():
        for _ in tf.range(steps):
            res = test_fn(tf.constant(args.times))
