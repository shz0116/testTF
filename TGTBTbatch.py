...
feature_watched_values = np.random.randint(0, features, (batch, nnz))
# feature_watched_values = np.array(list(range(batch * nnz)), dtype=np.float64)
...
#                               dtype=dtypes.int32))
      ...
      return dataset_ops.DatasetV2.from_tensor_slices(features).repeat().batch(batch) # batch_size
    return input_fn
