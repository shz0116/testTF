...
if __name__ == "__main__":
  ...
  # with tf.device('TPU:0'):
    ...
    if m == n and m == k:
        ...
        print("t1 is ", t1)
        if tf.reduce_sum(tf.linalg.diag_part(x)).numpy() == min(m,k):
    ...
    z = measure(z, x, y, tf.constant(steps), min(m,k))
    # print(z)
    if tf.reduce_sum(tf.linalg.diag_part(x)).numpy() == min(m,k):
        print("Results are correct")
    else:
    ...
    # cf = measure.get_concrete_function(z, x, y, tf.constant(steps), min(k,n))
    # t1 = time.perf_counter()
